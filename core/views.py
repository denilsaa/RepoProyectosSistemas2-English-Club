...
from .forms import EstudianteFormTecnico, EstudianteFormRegular, TutorForm
    ...
    if request.method == "POST":
        ...
        with connection.cursor() as cursor:
            cursor.execute("""
                ...
                FROM usuario
                WHERE nombre_usuario = %s AND contrasena = %s AND rol = %s AND activo = 1
            """, [username, password, user_type])
            user = cursor.fetchone()

        if user:
            request.session['id_usuario'] = user[0]
            request.session['nombre_usuario'] = user[1]
...
def crear_estudiante_tecnico(request):
    ...
    return render(request, 'directivo/crear_usuario_estudiante_tecnico.html', {'form': form, 'today': date.today()})
...
def crear_estudiante_regular(request):
    ...
    return render(request, 'directivo/crear_usuario_estudiante_regular.html', {
        'form': form,
        'today': date.today(),
...
def crear_tutor(request):
    ...
    if request.method == 'POST' and form.is_valid():
        try:
            ...
            if id_est is None:
                messages.error(request, 'No se pudo registrar el estudiante. Proceso cancelado.')
                return redirect('crear_estudiante_regular')
            ...
            # Guardar los accesos en sesión para mostrarlos
            ...
            # Limpiar datos temporales del estudiante
    ...
    return render(request, 'directivo/crear_usuario_tutor.html', {'form': form})
...
def estudiante_registrado(request):
    ...
    return render(request, 'directivo/estudiante_registrado.html', {
        'usuario': usuario,
        'password': password
    })
...
def estudiante_tutor_registrado(request):
    ...
    return render(request, 'directivo/estudiante_tutor_registrado.html', {
